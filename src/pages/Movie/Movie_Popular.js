...
function MoviesPopular() {
    ...
    useEffect(() => {
        fetch(`${BASE_URL}/movie/popular?api_key=${API_KEY}&page=${pages}`)
            .then(respon => respon.json())
            .then(data => setMoviesPopular([...moviesPopular, ...data.results]))
    }, [pages])
    return (
        <div>
            <Grid>
                {moviesPopular.length !== 0 && moviesPopular.map((item, index) => {
                    return (
